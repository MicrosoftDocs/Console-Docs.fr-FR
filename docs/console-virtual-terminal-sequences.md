...
| ECHAP \[ &lt; n &gt; E | CNL | Ligne suivante du curseur | Curseur vers le dessous &lt; &gt; de n lignes à partir de la position actuelle |
| ECHAP \[ &lt; n &gt; F | PERSONNALISATION | Ligne précédente du curseur | Curseur vers le haut &lt; n &gt; lignes à partir de la position actuelle |
...
> [!TIP]
> Les séquences d’activation se terminent par un caractère H minuscule ( `h` ) et les séquences de désactivation se terminent par un caractère de L minuscule ( `l` ).
...
> [!NOTE]
>Ces requêtes émettent leurs réponses dans le flux d’entrée de la console immédiatement après leur reconnaissance sur le flux de sortie pendant que l’activation du \_ \_ traitement des terminaux virtuels \_ est définie. L' \_ \_ indicateur d’entrée activer le terminal virtuel \_ ne s’applique pas aux commandes de requête, car il est supposé qu’une application qui effectue la requête doit toujours recevoir la réponse.
...
> [!NOTE]
><kbd>CTRL</kbd> gauche + droite <kbd>ALT</kbd> est traité comme AltGr. Lorsqu’ils sont affichés ensemble, ils sont supprimés et la valeur Unicode du caractère présenté par le système est transmise à la cible. Le système convertit les valeurs AltGr en fonction des paramètres d’entrée système actuels.
...
> [!NOTE]
>Dans l’exemple précédent, la chaîne' `\x1b[31m` 'correspond à l’implémentation de **ESC \[ &lt; n &gt; m** avec &lt; n &gt; étant 31.
